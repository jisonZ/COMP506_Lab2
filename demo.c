...
struct NODE *make_leaf_node(int node_type, char *var_name, char const *var_value)
{
    ...
    {
        int int_val = atoi(var_value);
        n->var_type = node_type;
        n->var_val = int_val;
    }
    else if (node_type == TYPE_CHAR)
    {
        char char_val = var_value[1];
        n->var_type = node_type;
        n->var_val = char_val;
    }
    else
    {
        ...
        {
            // printf("\nnull leaf\n");
            printf("Variable %s at ", var_name);

            yyerror(" is an undeclared Variable");
            // die();
        }
    }
    // printf("makeing a leaf node %s,type %d,token %s,value %d:\n",var_name, node_type, var_value, n->var_val);
    n->child_num = 0;
...
{
    struct NODE *n = (struct NODE *)malloc(sizeof(struct NODE));
    // printf("makeing op node type:%d", node_type);
    n->node_type = node_type;
...
void walk(struct NODE *root)
{
    ...
    switch (root->node_type)
    {
        ...
        case NODE_ASSIGN:
            walk(root->children[1]);
            update_st(root->children[0]->var_name, root->children[1]->var_type, root->children[1]->var_val);
            return;
        case NODE_WRITE:
            walk(root->children[0]);
